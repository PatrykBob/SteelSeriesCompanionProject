...
namespace SteelSeriesSonarCompanion.Communication.Internal
{
	public static class InternalCommunicationAddressProvider
	{
		...
		private const string CLASSIC_VOLUME_ENDPOINT_FORMAT = "{0}:{1}/volumeSettings/classic/{2}/Volume/{3}";
		private const string CLASSIC_MUTE_ENDPOINT_FORMAT = "{0}:{1}/volumeSettings/classic/{2}/Mute/{3}";
		...
		{
			return new Uri(string.Format(CLASSIC_VOLUME_ENDPOINT_FORMAT, LOCALHOST_ADDRESS, port, channel, volume.ToString(CultureInfo.InvariantCulture)));
		}

		public static Uri GetClassicChannelMuteAddress (int port, string channel, bool mute)
		{
			return new Uri(string.Format(CLASSIC_MUTE_ENDPOINT_FORMAT, LOCALHOST_ADDRESS, port, channel, mute));
		}
